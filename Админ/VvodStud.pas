...
begin
 if Length(Trim(LabeledEdit1.Text)) = 0 then
  begin
    MessageDlg('Вы не ввели ФИО  Студента!',mtError,[mbOk],0);
    Exit     //Выход
  end;

    if Length(Trim(DBLookupComboBox1.Text)) = 0 then
  begin
    MessageDlg('Вы не выбрали группу!',mtError,[mbOk],0);
    Exit     //Выход
  end;
 ...
 if Length(Trim(ComboBox1.Text)) = 0 then
  begin
    MessageDlg('Вы не выбрали должность!',mtError,[mbOk],0);
    Exit     //Выход
  end;
...
MessageDlg('Новый Студент успешно добавлен.',mtInformation,[mbOK],0);  // Диалоговое окно.

FDMSN.ADOQueryUsar.Close;
FDMSN.ADOQueryUsar.Open;

DBGrid1.Columns.Items[0].Title.Caption :='Код Студента';
 DBGrid1.Columns.Items[1].Title.Caption :='ФИО';
  DBGrid1.Columns.Items[2].Title.Caption :='Должность';
   DBGrid1.Columns.Items[3].Title.Caption :='Пароль';
    DBGrid1.Columns.Items[4].Title.Caption :='Код Группы';
end;
