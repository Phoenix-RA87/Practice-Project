...
begin
 if Length(Trim(LabeledEdit1.Text)) = 0 then
  begin
    MessageDlg('Вы не ввели НазвГрупп!',mtError,[mbOk],0);
    Exit     //Выход
  end;
...
MessageDlg('Новая Группа успешно добавлена.',mtInformation,[mbOK],0);  // Диалоговое окно.

FDMSN.ADOQueryGruppa.Close;
FDMSN.ADOQueryGruppa.Open;

DBGrid1.Columns.Items[0].Title.Caption :='Код группы';
 DBGrid1.Columns.Items[1].Title.Caption :='НазвГрупп';
end;
