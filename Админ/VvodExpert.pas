...
begin
 if Length(Trim(LabeledEdit2.Text)) = 0 then
   begin
     MessageDlg('Вы не указали ФИО!',mtError,[mbOk],0); // Диалоговое окно.
     Exit     //Выход
  end;

  if Length(Trim(LabeledEdit3.Text)) = 0 then
   begin
     MessageDlg('Вы не указали Пароль!',mtError,[mbOk],0); // Диалоговое окно.
     Exit     //Выход
  end;


// написать код, в котором   RadioGroup1.Items
// будет одно значение из нескольких записывать в параметр Status DBGrid
...
MessageDlg('Новый Эксперт успешно добавлен.',mtInformation,[mbOK],0);  // Диалоговое окно.

FDMSN.ADOQueryUsar.Close;
FDMSN.ADOQueryUsar.Open;

DBGrid2.Columns.Items[0].Title.Caption :='КодПользователя';
 DBGrid2.Columns.Items[1].Title.Caption :='ФИО';
  DBGrid2.Columns.Items[2].Title.Caption :='Должность';
   DBGrid2.Columns.Items[3].Title.Caption :='Пароль';
    DBGrid2.Columns.Items[4].Title.Caption :='КодГруппы';
end;                              

procedure TFSNVvodExpert.BitBtn2Click(Sender: TObject);
begin
if MessageDlg('Вернуться в Главное меню?', mtConfirmation,[mbOk, mbCancel], 0) = mrOk then   // Диалоговое окно
begin
FSNVvodExpert.Close;
end;
end;
...
begin
if MessageDlg('Закончить работу программы?', mtConfirmation,[mbOk, mbCancel], 0) = mrOk then    // Диалоговое окно
begin
Close;
